...
class InputUnit(nn.Module):
    def __init__(self,
        ...
        if self.question_model == 'RNN' or self.question_model == 'Transformer':
            self.question_encoder = nn.LSTM(self.dim, rnn_dim, batch_first=True, bidirectional=bidirectional)
        
        if self.question_model == 'Transformer' or self.question_model == 'BERT':
            context_encoder_layer = nn.TransformerEncoderLayer(self.dim, num_transformer_heads)
            self.context_encoder = nn.TransformerEncoder(context_encoder_layer, num_transformer_layers)

        if self.question_model == 'BERT':
            self.vocab_size += 1

        if self.separate_syntax_semantics_embeddings:
            wordvec_dim *= 2
        
        self.encoder_embed = nn.Embedding(self.vocab_size, self.dim)
    ...
    def forward(self, image, question, question_len):
        ...
        if self.question_model == 'BERT':
            question = torch.cat([question, torch.tensor(self.vocab_size -1).repeat(question.size(0), 1)], dim=-1 )
        ...
        if self.question_model == 'RNN' or self.question_model == 'Transformer':
            embed_pad = nn.utils.rnn.pack_padded_sequence(embed, question_len, batch_first=True)
            contextual_words, (question_embedding, _) = self.question_encoder(embed_pad)
        ...
        if self.question_model == 'Transformer' or self.question_model == 'BERT':
            contextual_words = self.context_encoder(embed.permute(1, 0, 2)).permute(1, 0, 2)

        if self.question_model == 'BERT':
            question_embedding = contextual_words[:, -1, :]
        print(question_embedding.shape, contextual_words.shape)
